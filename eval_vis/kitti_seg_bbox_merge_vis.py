...
#out dir
out_root = "./merge_ply_vis_pbbox_val/"
...
is_vis_pred_bbox = True
bbox_pred_color = (20,80,255)
...
for scene_id in scene_list:
    ...
    for fru_pred_file in fru_pred_list:
        ...
        if is_vis_pred_bbox:
            ...
            bbox_file =  bbox_root + file_id + ".txt" 
            ...
            if os.path.exists(bbox_file):
                ...
                with open(avg_file,"r") as f:
                    for line in f.readlines():
                        avg_dic[line.split(" ")[0]] = float(line.split(" ")[1].strip())
                ...
                with open(bbox_file,"r") as f:
                    for line in f.readlines():
                        ...
                        if line_s[0] == "hwl":
                            hwl_rel = [float(value) for value in line_s[1:]]

                        elif line_s[0] == "xyz":
                            xyz = [float(value) for value in line_s[1:]]

                        elif line_s[0] == "ry":
                            ry_cls = int(line_s[1].strip())

                #compute ry
                #cls to ry_relative
                ry_rel = (ry_cls - 12)* 15 * math.pi/180.0
        
                #ry_rel to ry
                ry_t = ry_rel - frustum_label_dic[file_id]
                ...
                h = hwl_rel[0] + avg_dic['avg_h']
                w = hwl_rel[1] + avg_dic['avg_w'] 
                l = hwl_rel[2] + avg_dic['avg_l']
                ...
                cp_pt_o = clockwiserot([x_f,z_f],[0,0],-frustum_label_dic[file_id])
    
                x = cp_pt_o[0]
                z = cp_pt_o[1]
    
                bbox_pred_center = [x,y,z]
                ...
                bbp1 = clockwiserot([x-l/2,z+w/2],pt_c,ry)
                bbp2 = clockwiserot([x+l/2,z+w/2],pt_c,ry)
                bbp3 = clockwiserot([x+l/2,z-w/2],pt_c,ry)
                bbp4 = clockwiserot([x-l/2,z-w/2],pt_c,ry)
    
                bbox_pred_xz = [bbp1,bbp2,bbp3,bbp4]
    
                #vis center
                pts_bbox_pred.append(bbox_pred_center)
                seg_bbox_pred.append(bbox_pred_color)
    
                #vis bbox
                p1 = [bbox_pred_xz[0][0],bbox_pred_miny,bbox_pred_xz[0][1]]
                p2 = [bbox_pred_xz[1][0],bbox_pred_miny,bbox_pred_xz[1][1]]
                p3 = [bbox_pred_xz[2][0],bbox_pred_miny,bbox_pred_xz[2][1]]
                p4 = [bbox_pred_xz[3][0],bbox_pred_miny,bbox_pred_xz[3][1]]
                p5 = [bbox_pred_xz[0][0],bbox_pred_maxy,bbox_pred_xz[0][1]]
                p6 = [bbox_pred_xz[1][0],bbox_pred_maxy,bbox_pred_xz[1][1]]
                p7 = [bbox_pred_xz[2][0],bbox_pred_maxy,bbox_pred_xz[2][1]]
                p8 = [bbox_pred_xz[3][0],bbox_pred_maxy,bbox_pred_xz[3][1]]
    ...
    if is_vis_bbox:
        ...
        seg_ply_gt_save = seg_ply_bbox + seg_ply_gt_save
    if is_vis_pred_bbox:
        pts_ply_gt_save = pts_bbox_pred + pts_ply_gt_save
        seg_ply_gt_save = seg_bbox_pred + seg_ply_gt_save
